...
#         cv2.waitKey(0)
        ...
        # HSV -> Green Mask
        # At different stages, may need to change numbers
        lower_green = np.array([24, 50, 50])   # [ 35 , 22-17 , 24-20 ] for yellows instead of greens
        upper_green = np.array([52, 205, 205]) # [ 95 , 52-17 , 52-45 ] for yellows instead of greens
        maskGreen = cv2.inRange(imgHSV, lower_green, upper_green)
#         cv2.imshow('mask', maskGreen)
#         cv2.waitKey(0)

        # Green Mask -> Blurred
        blurred = cv2.bilateralFilter(maskGreen,9,80,80)
...
#         cv2.waitKey(0)
        ...
        # Percentage of green pixels in image, used to construct y2
