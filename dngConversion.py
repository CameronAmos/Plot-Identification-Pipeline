...
#        rgb = raw.postprocess(demosaic_algorithm=None, use_auto_wb=True, output_color = rawpy.ColorSpace.Adobe, half_size=True)
        rgb = raw.postprocess(demosaic_algorithm=rawpy.DemosaicAlgorithm.LINEAR, output_color=rawpy.ColorSpace.sRGB, use_camera_wb=True, half_size=True) # Linear, sRGB half_size=False
        print(img)
        imageio.imsave(img.replace('.dng','.jpg'), rgb)
